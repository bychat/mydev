...
|------|-------------|-------------|
| `chatPrompts.ts` | Prompt builder wrappers for the chat agent pipeline | `buildSystemContext`, `buildResearchPrompt`, `buildActionPlanPrompt`, `buildFileChangePrompt`, `buildVerifyPrompt`, `buildCheckAgentPrompt` |
...
`core/chat.ts` contains the **shared** prompt builders used by all three runtimes (desktop, CLI, server). `chatPrompts.ts` wraps those with renderer-specific logic (e.g. attaching open tab context, workspace file paths). If you're adding a new prompt that needs to work in the CLI too, put it in `core/chat.ts`.
